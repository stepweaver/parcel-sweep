...
| `GOOGLE_ADDRESS_VALIDATION_API_KEY` | No | — | Server-only Address Validation API for Field Capture. Enable the API on the Google Cloud project first; the Geocoding key can be reused once Address Validation is enabled. App starts without it. |
| `GOOGLE_ADDRESS_VALIDATION_ENABLE_CASS` | No | `false` | Set `true` to request USPS CASS processing. Not required. |
| `OPENAI_API_KEY` | No | — | Field Capture transcription (`gpt-4o-transcribe`) and address parsing. Paste/heuristic split still work without it. |
